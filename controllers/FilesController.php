...
//use isalcedo\filemanager\models\FilesTag;
	...
	class FilesController extends Controller
	{
		...
		{
			if (!file_exists($model->storage_id . $model->url))
			{
				// File mode : 0755, Ref: http://php.net/manual/en/function.chmod.php
				mkdir($model->storage_id . $model->url, 0755, true);
			}
			if (!$file->saveAs($model->storage_id . $model->url . '/' . $model->src_file_name))
			{
				return [
					'status'    => false,
			...
			if ($model->dimension)
			{
				...
				$uploadThumbResult     = $this->createThumbnail($model, $model->storage_id . $model->url . '/' . $model->src_file_name);
